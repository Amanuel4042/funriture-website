		...
		>
			<div className='flex items-center justify-between pb-4 mb-6 border-b border-grey-100'>
				<h3 className='font-primary font-bold text-lg text-primary flex items-center gap-2'>
					...
					<span>{t("productsPage.sidebarTitle", "Filters")}</span>
				</h3>
				<button
					type='button'
				...
				>
					...
					<span>{t("productsPage.reset", "Reset")}</span>
				</button>
			</div>

			{/* Liked Items Quick Filter */}
			<div className='mb-6'>
				<button
					type='button'
					onClick={() => onToggleWishlistOnly && onToggleWishlistOnly(!wishlistOnly)}
					className={`w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all border ${
						wishlistOnly
							...
							: "bg-white border-grey-200 text-grey-700 hover:border-rose-200 hover:bg-rose-50/30"
					}`}
				>
					<span className='inline-flex items-center gap-2'>
						<IoIosHeart className={`text-base ${wishlistOnly ? "text-rose-500" : "text-grey-400"}`} />
						<span>{t("productsPage.likedOnlyToggle", "Liked Items Only")}</span>
					</span>
					<span
			...
			<div className='mb-7'>
				<h4 className='text-xs font-bold uppercase tracking-wider text-grey-500 mb-3'>
					{t("productsPage.categoriesTitle", "Categories")}
				</h4>
			...
			<div className='mb-7 pt-4 border-t border-grey-100'>
				<div className='flex items-center justify-between mb-2'>
					<h4 className='text-xs font-bold uppercase tracking-wider text-grey-500'>
						{t("productsPage.maxPriceTitle", "Max Price")}
					</h4>
					<span className='text-sm font-bold text-accent'>
						{maxPrice.toLocaleString()} {currencyText}
					</span>
				</div>
				<input
					type='range'
				...
				<div className='flex justify-between text-[11px] text-grey-400 mt-1'>
					<span>10,000 {currencyText}</span>
					<span>100,000 {currencyText}</span>
				</div>
			</div>
			...
			<div className='mb-6 pt-4 border-t border-grey-100'>
				<h4 className='text-xs font-bold uppercase tracking-wider text-grey-500 mb-3'>
					{t("productsPage.materialTitle", "Material")}
				</h4>
					...
					{materialsList.map((mat) => {
						...
						return (
							...
							>
								...
								/>
								<span>{translateMaterial(mat)}</span>
							</label>
						);
					})}
				</div>
			...
			{isOpenMobile && (
				...
				>
					{t("productsPage.applyFilters", "Apply Filters")}
				</button>
			)}
		</aside>
	);
